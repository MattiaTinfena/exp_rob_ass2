	...
	void do_work() override {
		...
		if (args.size() == 0) {
			RCLCPP_ERROR(get_logger(),
						 "Not enough arguments for capturing first image");
...
int main(int argc, char **argv) {
	...
	capture_first_node->set_parameter(
		rclcpp::Parameter("action_name", "capture_first_img"));
	capture_first_node->trigger_transition(
